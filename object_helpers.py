...
class SaveSelectedObject:
    ...
    def save_selected(cls, context, file_path, blender_path, to_world_origin, cleanup_startup_file):
        ...
        # files path
        temp_py_path = os.path.join(
            temp_dir, cls._tmp_name + '.py').replace('\\', '/')
        temp_blend_path = os.path.join(
            temp_dir, cls._tmp_name + '.blend').replace('\\', '/')

        dest_path = os.path.join(
            _objects, f"{uuid.uuid4()}.blend"
        ).replace('\\', '/')
        ...
        text_block_content += 'dest_path="' + dest_path + '"' + '\n'
